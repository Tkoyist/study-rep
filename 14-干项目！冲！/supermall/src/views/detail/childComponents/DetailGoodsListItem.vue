<template>
    <div class="goodsbox" @click="goodsClick">
      <!-- 在vue中，img 标签存在一个事件@load 在img 标签中的图片加载完成之后，就会触发该事件 -->
        <img :src="goodsitems.image" alt="" @load="imgLoad">
        <div class="goodsinfo">
...
export default {
    ...
    methods:{
      imgLoad(){
        // 我们在detail 页面中也会触发imgload 事件，由于我们是通过事件总线传递事件，所以我们发送的事件也会被home 页面监听到，这是会降低效率的
        // 为了避免这种状况，我们修改detail 的goodsitem 传给事件总线的事件
        this.$bus.$emit("DetailImgLoadEnd")
      },
      goodsClick(){
